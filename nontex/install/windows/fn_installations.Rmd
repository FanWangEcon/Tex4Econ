...
Go back to [fan](http://fanwangecon.github.io/)'s [Tex4Econ and Miscellaneous Repository](http://fanwangecon.github.io/Tex4Econ/). This file contains various excessive details on installations. 
...
2. Python installable packages and modules created with poetry, push to github as pypi: Locally test specially functions in [poetry](https://python-poetry.org/docs/#installation) environment. Push to github and Pypi, and then install via pip for system wide use.
...
For development machines, install [Poetry](https://github.com/python-poetry/poetry) and nstall [sphinx](https://www.sphinx-doc.org/en/master/).
...
### Poetry Update Python Version 

[poetry change python version to 3.x](https://stackoverflow.com/questions/60580113/poetry-change-python-version-to-3-x)

The existing poetry env needs to be deleted and re-created if need to update python version. 

```{bash}
poetry env list 
```

1. Update python requirement in .toml to *python = "^3.8"*
2. delete current env: *poetry env remove python*
3. run: *poetry lock*
4. run: *poetry update*, *poetry install*, etc...

### Installating Poetry Created Packages 
...
### Developing and testing new tools Pycharm + Poetry
...
[sphinx readthedocx](https://samnicholls.net/2016/06/15/how-to-sphinx-readthedocs/)

SEE BELOW *pyfan/pyecon workflow and files added*:
...
mkdir docs -->
cd doc/
rm -rf _build/ auto_examples/ gen_modules/backreferences gen_modules/generators
...
SEE BELOW *pyfan/pyecon workflow and files added*, on some local computer install pyfan:
...
##### pyfan/pyecon workflow and files added
...
2. Link to py functions if implemented there
3. Run file internally
4. Add new files to *_bookdown.yml*
5. Run pyfan rmd compile
6. Overall bookdown check pdf and html
...
cd "C:/Users/fan/pyfan/doc/"
cd "C:/Users/fan/pyecon/doc"
rm -rf _build/ auto_examples/ gen_modules/backreferences gen_modules/generators
make html

<!-- Check Results in: C:\Users\fan\pyfan\doc\_build\html\index.html -->
...
pip install works inside conda env. Uninstall first. Then install pyfan in conda env using pip. With user option, seems to install system-wide. 

Note that if some of the conda env use different python versions than base, then when base programs are updated, will not update in env. 

Might need to update this in all sub-directories

```{bash}
<!-- local install, this does not install in conda env -->
cd "C:/Users/fan/pyfan/"
cd "C:/Users/fan/pyecon/"
python setup.py install --user
<!-- after installing show appear under pip: pip show pyfan, with latest version ID -->
<!-- after installing this does not appear on conda list: conda list -n base, but in python, import pyfan as pyfan works -->
<!-- help('modules') in python to see if installed-->
...
<!-- if pyfan already installed, but not in conda env, uninstall and installing puts it in conda env -->
pip uninstall pyfan -y
pip install pyfan
<!-- --user installs it for outside of env -->
pip install pyfan --user
...
- [phinx-gallery](https://sphinx-gallery.github.io/stable/index.html)
...
# normal update
# to show all currently installed: conda list -n base
conda update --all
```
...
#### Conda Perl Environment

To run some perl tools in an isolated environment:

1. [latexpand](https://tex.stackexchange.com/a/21840/109818)
...
conda create -n wk_pyfan -y python=3.8
conda activate wk_pyfan
...
conda install -c conda-forge cdsapi -y
conda install -c conda-forge eccodes -y
...
#### Conda ECMWF Envir Python 2

```

conda deactivate
conda list env
conda env remove -n wk_ecmwf_py2
conda create -n wk_ecmwf_py2 python=2.7 -y 
conda activate wk_ecmwf_py2

python --version
```

#### Conda CGE Environment

Installing pip installed/updated python version from 3.7 to 3.8.

```
conda deactivate
conda list env
conda env remove -n wk_cgefi
conda create -n wk_cgefi -y
conda activate wk_cgefi
...
conda install -c conda-forge interpolation -y
...
AWS environment and set-up on local computer
...
- [reticulate](https://github.com/rstudio/reticulate)
    + install directly from: devtools::install_github("rstudio/reticulate")
...
1. go to Mathworks, download the latest file.s 

# Word

[Change background color and text color in word](https://www.davidbosman.fr/scsaw/?p=687)

Use these for my preferred background and text colors, consistent with SumatraPDF

> TextColor = #1e170f, R=30, G=23, B=15
> BackgroundColor = #fff8e6, R=255, G=248, B=230
...
1. Notepad++: for sync with Sumatra and quick edit by section, and fast quick editor in general
2. 
...
Download [Spell Right](https://marketplace.visualstudio.com/items?itemName=ban.spellright).
...
### Atom Shortcuts, Features and Settings
...
- Hard Wrap Text: *Ctrl + Shift + q*

**Spellcheck Scope** :

- cmd+shift+p, run *Editor: Log Cursor Scope*, see notification that comes up, add the *scopes* found to the *spell-check* package scope list. 
...
- clear all console outputs, collapse all: *edit + clear all outputs*
...
### PyCharm Settings GUI View etc

Change Theme, modify by changing plug-in.

Go to Settings, *Ctrl + Alt + S*, inside settings, go to theme/appearances, and chagne which theme is used. Material theme has a number of different options.

- Change where Tabs are displayed top or side: Settings -> Editor -> Editor Tabs -> Tab Placement (right)
- To change font: Settings -> Color Scheme -> Color Scheme Font ->

### PyCharm Set Virtual Env


1. In conda env: `where python`: `C:\ProgramData\Anaconda3\envs\wk_cgefi\python.exe`
2. In Pycharm: Settings (ctrl+alt+s), Project, Python interpreter.
3. Click on the cog button, "Add" > Right menu, "Virtual Environement" > Existing environement > Three dots > paste the path and point it to the python.exe in Scripts
...
- copy current function/line reference: *Ctrl + Shift + Alt + C*
- copy current file full path: *Ctrl + Shift + C*

**run/debug/test**

- Run current file: *ctrl+shift+f10*
- Run all tests in current file: *alt+shift+f10*
- Clear all Break-Points: *Ctrl+Shift+F8, Delete*
- Open up evaluator during debug: *Alt + F8*
- right click on a file and choose to run all "pytest", then should start test x of N

**Coding**

[code editor help](https://www.jetbrains.com/help/pycharm/using-code-editor.html)

- Refractor Names: *SHIFT + F6*
- Auto-Indent: *CTRL + SHIFT + ALT + L*
- Upper Case All: "CTRL + SHFT + U"
- Lower Case All: "CTRL + SHFT + L"
- reformat auto-indent: "Ctrl + Alt + L"
- Highlight searched: "Ctrl + Shift + F7"

**GUI**

Switching between panes:

1. set up default: Windows - Store Current as Default, go to default with *Shift + F12*
2. back to editor main: *CTRL + SHIFT + F12*

- Hide All non-code Windows: *CTRL + SHIFT + F12*
- Shift to last active: *F12*
- Maximize Output/Console Pane: *CTRL + SHIFT + QUOTE*
- Hide Output Pane: *SHIFT + ESC*
- Show Console output max: first debug, where debug is activate, *Ctrl + Shift + '*

**Settings**

- Go to Settings: *Ctrl + Alt + S*, click on keymap
...
- Install Notepad++
- Settings + Preferences + Cloud + Google Drive/App/Notepad++ Sync Setting there. 
- install [Npp-1-Dark](https://github.com/60ss/Npp-1-Dark)
...
- [nodejs](https://nodejs.org/en/) (revealjs pdf conversion)
- 7-zip
- evernote
